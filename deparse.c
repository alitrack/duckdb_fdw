...
/*
 * Context for duckdb_deparse_expr
 */
typedef struct deparse_expr_cxt
{
	...
	Expr	   *complementarynode;	/* variable where we can store, only if
									 ...
									 * Created mostly for
									 * duckdb_deparse_op_expr to have both
 ...
 * remote server.
 */
static bool duckdb_foreign_expr_walker(Node *node,
 ...
 */
static void duckdb_deparse_expr(Expr *expr, deparse_expr_cxt *context);
static void duckdb_deparse_var(Var *node, deparse_expr_cxt *context);
static void duckdb_deparse_const(Const *node, deparse_expr_cxt *context, int showtype);
static void duckdb_deparse_param(Param *node, deparse_expr_cxt *context);
static void duckdb_deparse_func_expr(FuncExpr *node, deparse_expr_cxt *context);
static void duckdb_deparse_op_expr(OpExpr *node, deparse_expr_cxt *context);
static void duckdb_deparse_operator_name(StringInfo buf, Form_pg_operator opform);

static void duckdb_deparse_scalar_array_op_expr(ScalarArrayOpExpr *node,
												deparse_expr_cxt *context);
static void duckdb_deparse_relabel_type(RelabelType *node, deparse_expr_cxt *context);
static void duckdb_deparse_bool_expr(BoolExpr *node, deparse_expr_cxt *context);
static void duckdb_deparse_null_test(NullTest *node, deparse_expr_cxt *context);
static void duckdb_deparse_array_expr(ArrayExpr *node, deparse_expr_cxt *context);
static void duckdb_print_remote_param(int paramindex, Oid paramtype, int32 paramtypmod,
									  deparse_expr_cxt *context);
static void duckdb_print_remote_placeholder(Oid paramtype, int32 paramtypmod,
											deparse_expr_cxt *context);
static void duckdb_deparse_relation(StringInfo buf, Relation rel);
static void duckdb_deparse_target_list(StringInfo buf, PlannerInfo *root, Index rtindex, Relation rel,
									   Bitmapset *attrs_used, bool qualify_col, List **retrieved_attrs, bool is_concat, bool check_null);
static void duckdb_deparse_column_ref(StringInfo buf, int varno, int varattno, PlannerInfo *root, bool qualify_col);
static void duckdb_deparse_select(List *tlist, List **retrieved_attrs, deparse_expr_cxt *context);
static void duckdb_deparse_case_expr(CaseExpr *node, deparse_expr_cxt *context);
static void duckdb_deparse_null_if_expr(NullIfExpr *node, deparse_expr_cxt *context);
static void duckdb_deparse_coalesce_expr(CoalesceExpr *node, deparse_expr_cxt *context);
static void duckdb_deparse_from_expr_for_rel(StringInfo buf, PlannerInfo *root, RelOptInfo *foreignrel,
											 bool use_alias, Index ignore_rel, List **ignore_conds,
											 List **params_list);
static void duckdb_deparse_range_tbl_ref(StringInfo buf, PlannerInfo *root,
										 RelOptInfo *foreignrel, bool make_subquery,
										 Index ignore_rel, List **ignore_conds, List **params_list);
static void duckdb_deparse_from_expr(List *quals, deparse_expr_cxt *context);
static void duckdb_deparse_aggref(Aggref *node, deparse_expr_cxt *context);
static void duckdb_append_limit_clause(deparse_expr_cxt *context);
static void duckdb_append_conditions(List *exprs, deparse_expr_cxt *context);
static void duckdb_append_group_by_clause(List *tlist, deparse_expr_cxt *context);
static void duckdb_append_agg_order_by(List *orderList, List *targetList,
									   deparse_expr_cxt *context);
static void duckdb_append_order_by_clause(List *pathkeys, bool has_final_sort, deparse_expr_cxt *context);
static void duckdb_append_function_name(Oid funcid, deparse_expr_cxt *context);
const char *duckdb_get_jointype_name(JoinType jointype);
static Node *duckdb_deparse_sort_group_clause(Index ref, List *tlist, bool force_colno,
											  deparse_expr_cxt *context);
static void duckdb_deparse_explicit_target_list(List *tlist, List **retrieved_attrs,
												deparse_expr_cxt *context);
static void duckdb_deparse_minmax_expr(MinMaxExpr *node, deparse_expr_cxt *context);

/*
 * Helper functions
 */
static bool duckdb_is_subquery_var(Var *node, RelOptInfo *foreignrel,
								   int *relno, int *colno);
static void duckdb_get_relation_column_alias_ids(Var *node, RelOptInfo *foreignrel,
												 int *relno, int *colno);
static char *duckdb_quote_identifier(const char *s, char q);
 ...
 */
static void
duckdb_deparse_relation(StringInfo buf, Relation rel)
{
	ForeignTable *table;
	...
	{
		appendStringInfo(buf, "%s", duckdb_quote_identifier(relname, QUOTE));
	}
}
 ...
 */
bool
duckdb_is_foreign_expr(PlannerInfo *root,
					   RelOptInfo *baserel,
					   Expr *expr)
{
	foreign_glob_cxt glob_cxt;
 ...
 */
bool
duckdb_is_foreign_param(PlannerInfo *root,
						RelOptInfo *baserel,
						Expr *expr)
{
	if (expr == NULL)
 ...
 */
static bool
duckdb_foreign_expr_walker(Node *node,
						   foreign_glob_cxt *glob_cxt,
						   foreign_loc_cxt *outer_cxt)
{
		...
		case T_Const:
			{
				...
				/* DuckDB cannot handle interval type */
		...
		case T_Param:
			{
				...
				if (!duckdb_is_valid_type(p->paramtype))
					return false;
		...
		case T_FuncExpr:
			{
				...
				/*
				 * These function can be passed to DuckDB. In case of nest
				 ...
				 *
				 * In DuckDB, lower/upper function does not support UNICODE
				...
				if (!duckdb_foreign_expr_walker((Node *) func->args,
												glob_cxt, &inner_cxt))
					return false;
		...
		case T_NullIfExpr:
			{
				...
				/* ILIKE cannot be pushed down to DuckDB */
		...
		case T_RelabelType:
			{
				 ...
				 */
				if (!duckdb_foreign_expr_walker((Node *) r->arg,
												glob_cxt, &inner_cxt))
					return false;
		...
		case T_BoolExpr:
			{
				 ...
				 * Recurse to input subexpressions.
				 */
				if (!duckdb_foreign_expr_walker((Node *) b->args,
												glob_cxt, &inner_cxt))
					return false;
		...
		case T_NullTest:
			{
				 ...
				 * Recurse to input subexpressions.
				 */
				if (!duckdb_foreign_expr_walker((Node *) nt->arg,
												glob_cxt, &inner_cxt))
					return false;
		...
		case T_List:
			{
				...
				foreach(lc, l)
				{
					if (!duckdb_foreign_expr_walker((Node *) lfirst(lc),
													glob_cxt, &inner_cxt))
						return false;
				}
		...
		case T_CaseWhen:
			{
				...
				if (!duckdb_foreign_expr_walker((Node *) whenExpr->expr,
												glob_cxt, &inner_cxt))
					return false;
				/* Recurse to result expression. */
				if (!duckdb_foreign_expr_walker((Node *) whenExpr->result,
												glob_cxt, &inner_cxt))
					return false;
		...
		case T_Aggref:
			{
				...
				/* these function can be passed to DuckDB */
				...
				foreach(lc, agg->args)
				{
					...
					if (!duckdb_foreign_expr_walker(n, glob_cxt, &inner_cxt))
						return false;
				}
		...
		case T_ArrayExpr:
			{
				 ...
				 * Recurse to input subexpressions.
				 */
				if (!duckdb_foreign_expr_walker((Node *) a->elements,
												glob_cxt, &inner_cxt))
					return false;
		...
		case T_MinMaxExpr:
			{
				...
				if (!duckdb_foreign_expr_walker((Node *) mmexpr->args,
												glob_cxt, &inner_cxt))
					return false;
	 ...
	 */
	if (check_type && !duckdb_is_builtin(exprType(node)))
		return false;
 ...
 */
List *
duckdb_build_tlist_to_deparse(RelOptInfo *foreignrel)
{
	List	   *tlist = NIL;
 ...
 */
void
duckdb_deparse_select_stmt_for_rel(StringInfo buf, PlannerInfo *root, RelOptInfo *rel,
...
{
	deparse_expr_cxt context;
	DuckDBFdwRelationInfo *fpinfo = (DuckDBFdwRelationInfo *) rel->fdw_private;
	List	   *quals;
	...
	duckdb_deparse_select(tlist, retrieved_attrs, &context);
	...
	if (IS_UPPER_REL(rel))
	{
		...
		ofpinfo = (DuckDBFdwRelationInfo *) fpinfo->outerrel->fdw_private;
	...
	duckdb_deparse_from_expr(quals, &context);

	if (IS_UPPER_REL(rel))
	{
		/* Append GROUP BY clause */
		duckdb_append_group_by_clause(tlist, &context);

		/* Append HAVING clause */
		if (remote_conds)
		{
			appendStringInfo(buf, " HAVING ");
			duckdb_append_conditions(remote_conds, &context);
		}
	}

	/* Add ORDER BY clause if we found any useful pathkeys */
	if (pathkeys)
		duckdb_append_order_by_clause(pathkeys, has_final_sort, &context);

	/* Add LIMIT clause if necessary */
	if (has_limit)
		duckdb_append_limit_clause(&context);
 ...
 */
static void
duckdb_deparse_select(List *tlist, List **retrieved_attrs, deparse_expr_cxt *context)
{
		...
		IS_UPPER_REL(foreignrel))
	{
		 ...
		 */
		duckdb_deparse_explicit_target_list(tlist, retrieved_attrs, context);
	}
	else
	{
		/*
		...
		duckdb_deparse_target_list(buf, root, foreignrel->relid, rel, fpinfo->attrs_used, false, retrieved_attrs, false, false);
 ...
 */
static void
duckdb_deparse_from_expr(List *quals, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	...
	duckdb_deparse_from_expr_for_rel(buf, context->root, scanrel,
	...
	if (quals != NIL)
	{
		...
		duckdb_append_conditions(quals, context);
	}
}
 ...
 */
static void
duckdb_append_conditions(List *exprs, deparse_expr_cxt *context)
{
	int			nestlevel;
	ListCell   *lc;
	bool		is_first = true;
	...
	nestlevel = duckdb_set_transmission_modes();
	...
	foreach(lc, exprs)
	{
		...
		appendStringInfoChar(buf, '(');
		duckdb_deparse_expr(expr, context);
		appendStringInfoChar(buf, ')');

		is_first = false;
	}

	duckdb_reset_transmission_modes(nestlevel);
}

/* Output join name for given join type */
const char *
duckdb_get_jointype_name(JoinType jointype)
{
	switch (jointype)
	{
		case JOIN_INNER:
 ...
 */
static void
duckdb_deparse_explicit_target_list(List *tlist, List **retrieved_attrs,
									deparse_expr_cxt *context)
{
	ListCell   *lc;
	StringInfo	buf = context->buf;
 ...
 */
static void
duckdb_deparse_from_expr_for_rel(StringInfo buf, PlannerInfo *root, RelOptInfo *foreignrel,
								 bool use_alias, Index ignore_rel, List **ignore_conds,
								 List **params_list)
{
	if (IS_JOIN_REL(foreignrel))
	{
		...
		if (!outerrel_is_target)
		{
			...
			duckdb_deparse_range_tbl_ref(&join_sql_o, root, outerrel,
		...
		if (!innerrel_is_target)
		{
			...
			duckdb_deparse_range_tbl_ref(&join_sql_i, root, innerrel,
		...
		appendStringInfo(buf, "(%s %s JOIN %s ON ", join_sql_o.data,
						 duckdb_get_jointype_name(fpinfo->jointype), join_sql_i.data);
		...
		if (fpinfo->joinclauses)
		{
			...
			duckdb_append_conditions(fpinfo->joinclauses, &context);
			appendStringInfo(buf, ")");
		}
		else
	...
	else
	{
		...
		duckdb_deparse_relation(buf, rel);
 ...
 */
static void
duckdb_deparse_range_tbl_ref(StringInfo buf, PlannerInfo *root, RelOptInfo *foreignrel,
							 ...
							 List **params_list)
{
	...
	if (make_subquery)
	{
		...
		appendStringInfoChar(buf, '(');
		duckdb_deparse_select_stmt_for_rel(buf, root, foreignrel, NIL,
		...
		duckdb_deparse_from_expr_for_rel(buf, root, foreignrel, true, ignore_rel,
 ...
 */
void
duckdb_deparse_insert(StringInfo buf, PlannerInfo *root,
...
#endif
	{
		...
		foreach(lc, targetAttrs)
		{
			...
			if (!attr->attgenerated)
			{
				...
				duckdb_deparse_column_ref(buf, rtindex, attnum, root, false);
#if PG_VERSION_NUM >= 140000
			}
#endif
		}
 ...
 */
void
duckdb_rebuild_insert(StringInfo buf, Relation rel, char *orig_query,
 ...
 */
void
duckdb_append_where_clause(StringInfo buf,
						   PlannerInfo *root,
						   RelOptInfo *baserel,
						   List *exprs,
						   bool is_first,
						   List **params)
{
	...
	foreach(lc, exprs)
	{
		...
		appendStringInfoChar(buf, '(');
		duckdb_deparse_expr(ri->clause, &context);
		appendStringInfoChar(buf, ')');
...
#if PG_VERSION_NUM >= 140000
/*
 * TRUNCATE in DuckDB is supported by use DELETE FROM without WHERE condition.
 */
void
duckdb_deparse_truncate(StringInfo buf,
						List *rels)
{
	ListCell   *cell;
	Relation	rel;
 ...
 */
static void
duckdb_deparse_column_ref(StringInfo buf, int varno, int varattno, PlannerInfo *root, bool qualify_col)
{
	RangeTblEntry *rte;
	...
	if (varattno == 0)
	{
		...
		duckdb_deparse_target_list(buf, root, varno, rel, attrs_used, qualify_col,
...
static char *
duckdb_deparse_column_option(int varno, int varattno, PlannerInfo *root, char *optionname)
{
	RangeTblEntry *rte;
...
*/
void
duckdb_deparse_string_literal(StringInfo buf, const char *val)
{
	const char *valptr;
 ...
 *
 * This function must support all the same node types that duckdb_foreign_expr_walker
 * accepts.
 *
 ...
 */
static void
duckdb_deparse_expr(Expr *node, deparse_expr_cxt *context)
{
	if (node == NULL)
		return;
	...
	{
		case T_Var:
			duckdb_deparse_var((Var *) node, context);
			break;
		case T_Const:
			duckdb_deparse_const((Const *) node, context, 0);
			break;
		case T_Param:
			duckdb_deparse_param((Param *) node, context);
			break;
		case T_FuncExpr:
			duckdb_deparse_func_expr((FuncExpr *) node, context);
			break;
		case T_OpExpr:
			duckdb_deparse_op_expr((OpExpr *) node, context);
			break;
		case T_ScalarArrayOpExpr:
			duckdb_deparse_scalar_array_op_expr((ScalarArrayOpExpr *) node, context);
			break;
		case T_RelabelType:
			duckdb_deparse_relabel_type((RelabelType *) node, context);
			break;
		case T_BoolExpr:
			duckdb_deparse_bool_expr((BoolExpr *) node, context);
			break;
		case T_NullTest:
			duckdb_deparse_null_test((NullTest *) node, context);
			break;
		case T_ArrayExpr:
			duckdb_deparse_array_expr((ArrayExpr *) node, context);
			break;
		case T_CaseExpr:
			duckdb_deparse_case_expr((CaseExpr *) node, context);
			break;
		case T_CoalesceExpr:
			duckdb_deparse_coalesce_expr((CoalesceExpr *) node, context);
			break;
		case T_NullIfExpr:
			duckdb_deparse_null_if_expr((NullIfExpr *) node, context);
			break;
		case T_MinMaxExpr:
			duckdb_deparse_minmax_expr((MinMaxExpr *) node, context);
			break;
		case T_Aggref:
			duckdb_deparse_aggref((Aggref *) node, context);
			break;
		default:
 ...
 */
void
duckdb_deparse_update(StringInfo buf, PlannerInfo *root,
					  Index rtindex, Relation rel,
 ...
 */
void
duckdb_deparse_direct_update_sql(StringInfo buf, PlannerInfo *root,
								 Index rtindex, Relation rel,
								 ...
								 List **retrieved_attrs)
{
	...
	if (IS_JOIN_REL(foreignrel))
	{
		...
		duckdb_deparse_from_expr_for_rel(buf, root, foreignrel, true, rtindex,
										 &ignore_conds, params_list);
 ...
 */
void
duckdb_deparse_delete(StringInfo buf, PlannerInfo *root,
					  Index rtindex, Relation rel,
					  List *attname)
{
	int			i = 0;
 ...
 */
void
duckdb_deparse_direct_delete_sql(StringInfo buf, PlannerInfo *root,
								 Index rtindex, Relation rel,
								 RelOptInfo *foreignrel,
								 List *remote_conds,
								 List **params_list,
 ...
 */
static void
duckdb_deparse_var(Var *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	 ...
	 */
	if (duckdb_is_subquery_var(node, context->scanrel, &relno, &colno))
	{
	...
	if (bms_is_member(node->varno, relids) && node->varlevelsup == 0)
	{
		...
		duckdb_deparse_column_ref(buf, node->varno, node->varattno, context->root, qualify_col);
	}
	else
	{
		/* Treat like a Param */
		if (context->params_list)
		{
			...
			duckdb_print_remote_param(pindex, node->vartype, node->vartypmod, context);
		}
		else
		{
			duckdb_print_remote_placeholder(node->vartype, node->vartypmod, context);
		}
	}
}
 ...
 * or not depending if duckdb column is TEXT or INT */
static Var *
 ...
 */
static void
duckdb_deparse_const(Const *node, deparse_expr_cxt *context, int showtype)
{
	StringInfo	buf = context->buf;
	Oid			typoutput;
	bool		typIsVarlena;
	char	   *extval;
	char	   *duckdbcolumntype;
	...
	switch (node->consttype)
	{
		...
		case BYTEAOID:
			 ...
			 * allows us to quickly convert postgres escaped strings to duckdb
			 * ones for comparison
			 */
			extval = OidOutputFunctionCall(typoutput, node->constvalue);
		...
		case TIMESTAMPOID:
			...
			if (context->complementarynode != NULL)
			{
				...
				if (varnode != NULL)
				{
					duckdbcolumntype = duckdb_deparse_column_option(varnode->varno, varnode->varattno, context->root, "column_type");

					if (duckdbcolumntype != NULL && strcmp(duckdbcolumntype, "INT") == 0)
			...
			if (convert_timestamp_tounixepoch)
				...
				duckdb_deparse_string_literal(buf, extval);
 ...
 */
static void
duckdb_deparse_param(Param *node, deparse_expr_cxt *context)
{
	if (context->params_list)
	{
		int			pindex = 0;
		...
		duckdb_print_remote_param(pindex, node->paramtype, node->paramtypmod, context);
	}
	else
	{
		duckdb_print_remote_placeholder(node->paramtype, node->paramtypmod, context);
	}
}

/*
 * This possible that name of function in PostgreSQL and
 * duckdb differ, so return the duckdb equelent function name
 */
static char *
duckdb_replace_function(char *in)
{
...
duckdb_deparse_func_expr(FuncExpr *node, deparse_expr_cxt *context)
{
	...
	foreach(arg, node->args)
	{
		if (!first)
			appendStringInfoString(buf, ", ");
		duckdb_deparse_expr((Expr *) lfirst(arg), context);
		first = false;
	}
	appendStringInfoChar(buf, ')');
	ReleaseSysCache(proctup);
...
duckdb_deparse_op_expr(OpExpr *node, deparse_expr_cxt *context)
{
	...
	if (oprkind == 'b')
	{
		...
		duckdb_deparse_expr(linitial(node->args), context);
		appendStringInfoChar(buf, ' ');
	}

	/* Deparse operator name. */
	duckdb_deparse_operator_name(buf, form);
	...
	duckdb_deparse_expr(llast(node->args), context);
 ...
 */
static void
duckdb_deparse_operator_name(StringInfo buf, Form_pg_operator opform)
{
	char	   *cur_opname = NULL;
	...
	if (opform->oprnamespace != PG_CATALOG_NAMESPACE)
	{
		...
		appendStringInfo(buf, "OPERATOR(%s.%s)",
						 duckdb_quote_identifier(opnspname, QUOTE), cur_opname);
	}
	else
	{
		if (strcmp(cur_opname, "~~") == 0)
 ...
 */
static void
duckdb_deparse_scalar_array_op_expr(ScalarArrayOpExpr *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	HeapTuple	tuple;
	Expr	   *arg1;
	...
	{
		/* Deparse left operand. */
		duckdb_deparse_expr(arg1, context);
		appendStringInfoChar(buf, ' ');
		...
		case T_ArrayExpr:
			{
				...
				foreach(lc, ((ArrayExpr *) arg2)->elements)
				{
					...
					if (useIn)
					{
						duckdb_deparse_expr(lfirst(lc), context);
					}
					else
					{
 ...
 */
static void
duckdb_deparse_relabel_type(RelabelType *node, deparse_expr_cxt *context)
{
	duckdb_deparse_expr(node->arg, context);
}
 ...
 */
static void
duckdb_deparse_bool_expr(BoolExpr *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	...
	foreach(lc, node->args)
	{
		...
		duckdb_deparse_expr((Expr *) lfirst(lc), context);
		first = false;
	}
	appendStringInfoChar(buf, ')');
}
 ...
 */
static void
duckdb_deparse_null_test(NullTest *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
 ...
 */
static void
duckdb_deparse_array_expr(ArrayExpr *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	bool		first = true;
	ListCell   *lc;
 ...
 */
static void
duckdb_deparse_case_expr(CaseExpr *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	ListCell   *lc = NULL;
	...
	foreach(lc, node->args)
	{
		...
		if (node->arg == NULL)	/* CASE WHEN */
			duckdb_deparse_expr(whenclause->expr, context);
		else					/* CASE arg WHEN */
			duckdb_deparse_expr(lsecond(((OpExpr *) whenclause->expr)->args), context);

		/* THEN */
		appendStringInfoString(buf, " THEN ");
		duckdb_deparse_expr(whenclause->result, context);
	}
	...
	if (node->defresult != NULL)
	{
		...
		duckdb_deparse_expr(node->defresult, context);
	}
 ...
 */
static void
duckdb_deparse_null_if_expr(NullIfExpr *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;

	appendStringInfoString(buf, "NULLIF(");
	duckdb_deparse_expr(lfirst(list_head(node->args)), context);
	appendStringInfoString(buf, ", ");
	duckdb_deparse_expr(lfirst(list_tail(node->args)), context);
	appendStringInfoChar(buf, ')');
}
 ...
 */
static void
duckdb_deparse_coalesce_expr(CoalesceExpr *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	ListCell   *lc;
	bool		first = true;
 ...
 */
static void
duckdb_deparse_minmax_expr(MinMaxExpr *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	ListCell   *lc;
	bool		first = true;
 ...
 */
static void
duckdb_print_remote_param(int paramindex, Oid paramtype, int32 paramtypmod,
						  deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
...
static void
duckdb_print_remote_placeholder(Oid paramtype, int32 paramtypmod,
								deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
 ...
 */
static void
duckdb_deparse_aggref(Aggref *node, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	bool		use_variadic;
	...
	duckdb_append_function_name(node->aggfnoid, context);
	appendStringInfoChar(buf, '(');
	...
	if (AGGKIND_IS_ORDERED_SET(node->aggkind))
	{
		...
		duckdb_append_agg_order_by(node->aggorder, node->args, context);
	}
	else
	{
		...
		else
		{
			...
			foreach(arg, node->args)
			{
				...
				duckdb_deparse_expr((Expr *) n, context);
			}
		}
	...
	if (node->aggfilter != NULL)
	{
		...
		duckdb_deparse_expr((Expr *) node->aggfilter, context);
	}
 ...
 */
static void
duckdb_append_group_by_clause(List *tlist, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	...
	foreach(lc, query->groupClause)
	{
		...
		duckdb_deparse_sort_group_clause(grp->tleSortGroupRef, tlist, true, context);
	}
}
 ...
 */
static void
duckdb_append_agg_order_by(List *orderList, List *targetList, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	ListCell   *lc;
	bool		first = true;
	...
	foreach(lc, orderList)
	{
		...
		sortexpr = duckdb_deparse_sort_group_clause(srt->tleSortGroupRef, targetList, false,
													context);
		...
		else
		{
			...
			duckdb_deparse_operator_name(buf, operform);
 ...
 */
static void
duckdb_append_order_by_clause(List *pathkeys, bool has_final_sort, deparse_expr_cxt *context)
{
	ListCell   *lcell;
	int			nestlevel;
	char	   *delim = " ";
	...
	foreach(lcell, pathkeys)
	{
		...
		if (has_final_sort)
		{
			 ...
			 * the final sort.
			 */
			em_expr = duckdb_find_em_expr_for_input_target(context->root,
														   pathkey->pk_eclass,
														   context->foreignrel->reltarget,
														   baserel);
		}
		else
			em_expr = duckdb_find_em_expr_for_rel(pathkey->pk_eclass, baserel);

		Assert(em_expr != NULL);

		appendStringInfoString(buf, delim);
		duckdb_deparse_expr(em_expr, context);
		...
		/*
		 * In DUCKDB3 Release v3.30.0 (2019-10-04) NULLS FIRST/LAST is
		 * supported, but not in prior versions More info:
		 * https://www.duckdb.org/changes.html
		 * https://www.duckdb.org/lang_select.html#orderby
		 */
		        		/* DuckDB 1.0+ supports NULLS FIRST/LAST */
		        		if (pathkey->pk_nulls_first)
		        			appendStringInfoString(context->buf, " NULLS FIRST");
		        		else
		        			appendStringInfoString(context->buf, " NULLS LAST");
		        		delim = ", ";
	}
	duckdb_reset_transmission_modes(nestlevel);
}
 ...
 */
static void
duckdb_append_limit_clause(deparse_expr_cxt *context)
{
	PlannerInfo *root = context->root;
	StringInfo	buf = context->buf;
	int			nestlevel;
	...
	if (root->parse->limitCount)
	{
		...
		duckdb_deparse_expr((Expr *) root->parse->limitCount, context);
	}
	else
	{
		/*
		 * We add this LIMIT -1 because OFFSET by itself its not
		 * implemented/allowed in DuckDB. You need to provide LIMIT *always*
		 * when using OFFSET
		 */
	...
	if (root->parse->limitOffset)
	{
		...
		duckdb_deparse_expr((Expr *) root->parse->limitOffset, context);
	}
...
/*
 * duckdb_append_function_name
 *		Deparses function name from given function oid.
 */
static void
duckdb_append_function_name(Oid funcid, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	HeapTuple	proctup;
	Form_pg_proc procform;
 ...
 */
static Node *
duckdb_deparse_sort_group_clause(Index ref, List *tlist, bool force_colno, deparse_expr_cxt *context)
{
	StringInfo	buf = context->buf;
	TargetEntry *tle;
	Expr	   *expr;
	...
	else if (expr && IsA(expr, Const))
	{
		 ...
		 * a constant.
		 */
		duckdb_deparse_const((Const *) expr, context, 1);
	}
 ...
 */
static bool
duckdb_is_subquery_var(Var *node, RelOptInfo *foreignrel, int *relno, int *colno)
{
	DuckDBFdwRelationInfo *fpinfo = (DuckDBFdwRelationInfo *) foreignrel->fdw_private;
	RelOptInfo *outerrel = fpinfo->outerrel;
	RelOptInfo *innerrel = fpinfo->innerrel;
	...
	if (bms_is_member(node->varno, outerrel->relids))
	{
		...
		{
			duckdb_get_relation_column_alias_ids(node, outerrel, relno, colno);
			return true;
		}

		/* Otherwise, recurse into the outer relation. */
		return duckdb_is_subquery_var(node, outerrel, relno, colno);
	}
	else
	{
		...
		{
			duckdb_get_relation_column_alias_ids(node, innerrel, relno, colno);
			return true;
		}

		/* Otherwise, recurse into the inner relation. */
		return duckdb_is_subquery_var(node, innerrel, relno, colno);
	}
}
 ...
 */
static void
duckdb_get_relation_column_alias_ids(Var *node, RelOptInfo *foreignrel,
									 int *relno, int *colno)
{
	DuckDBFdwRelationInfo *fpinfo = (DuckDBFdwRelationInfo *) foreignrel->fdw_private;
	int			i;
 ...
 */
static bool
duckdb_contain_immutable_functions(Node *clause)
{
	return duckdb_contain_immutable_functions_walker(clause, NULL);
}

static bool
duckdb_contain_immutable_functions_walker(Node *node, void *context)
{
	if (node == NULL)
		return false;
	...
	if (IsA(node, Query))
	{
		...
		return query_tree_walker((Query *) node,
								 duckdb_contain_immutable_functions_walker,
								 context, 0);
	}
	return expression_tree_walker(node, duckdb_contain_immutable_functions_walker,
								  context);
}
 ...
 */
bool
duckdb_is_foreign_function_tlist(PlannerInfo *root,
								 RelOptInfo *baserel,
								 List *tlist)
{
	foreign_glob_cxt glob_cxt;
	...
	foreach(lc, tlist)
	{
		...
		if (duckdb_contain_immutable_functions((Node *) tle->expr))
		{
	...
	foreach(lc, tlist)
	{
		...
		if (!duckdb_foreign_expr_walker((Node *) tle->expr, &glob_cxt, &loc_cxt))
			return false;
