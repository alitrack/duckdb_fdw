...
DO $d$
    BEGIN
        ...
        EXECUTE $$CREATE SERVER duckdb_svr2 FOREIGN DATA WRAPPER duckdb_fdw
            OPTIONS (database '/tmp/duckdbfdw_test_post.db')$$;
        EXECUTE $$CREATE SERVER duckdb_svr3 FOREIGN DATA WRAPPER duckdb_fdw
            OPTIONS (database '/tmp/duckdbfdw_test_post.db')$$;
    END;
$d$;
...
--Testcase 756:
CREATE USER MAPPING FOR public SERVER duckdb_svr3;
-- ===================================================================
...
CREATE FOREIGN TABLE ft7 (
	...
	c2 int NOT NULL,
	c3 text
) SERVER duckdb_svr3 OPTIONS (table 'T 4');
...
CREATE FOREIGN TABLE ft_empty (c1 int NOT NULL, c2 text)
  SERVER duckdb_svr OPTIONS (table 'loct_empty');
--Testcase 770:
...
CREATE FOREIGN TABLE reindex_foreign (c1 int, c2 int)
  SERVER duckdb_svr2 OPTIONS (table 'reindex_local');
...
CREATE FOREIGN TABLE reind_fdw_10_20 PARTITION OF reind_fdw_parent
  ...
  SERVER duckdb_svr OPTIONS (table 'reind_local_10_20');
...
create foreign table grem1 (
  ...
  server duckdb_svr options(table 'grem1_post14');
--Testcase 638:
explain (verbose, costs off)
...
-- test batch insert
--Testcase 826:
alter server duckdb_svr options (add batch_size '10');
--Testcase 827:
explain (verbose, costs off)
insert into grem1 (a) values (1), (2);
--Testcase 828:
...
delete from grem1;
--Testcase 831:
alter server duckdb_svr options (drop batch_size);
...
create foreign table foo2child (f3 int) inherits (foo2)
  server duckdb_svr options (table 'loct4_2');
...
CREATE FOREIGN TABLE tru_ftable (id int)
       SERVER duckdb_svr OPTIONS (table 'tru_rtable0');
--Testcase 851:
...
--        SERVER duckdb_svr OPTIONS (table 'tru_ptable');
...
CREATE FOREIGN TABLE tru_pk_ftable (id int)
       SERVER duckdb_svr OPTIONS (table 'tru_pk_table');

--Testcase 855:
CREATE FOREIGN TABLE tru_ftable_parent (id int)
       SERVER duckdb_svr OPTIONS (table 'tru_rtable_parent');
--Testcase 856:
CREATE FOREIGN TABLE tru_ftable_child () INHERITS (tru_ftable_parent)
       SERVER duckdb_svr OPTIONS (table 'tru_rtable_child');
--Testcase 857:
...
-- 'truncatable' option
--Testcase 862:
ALTER SERVER duckdb_svr OPTIONS (ADD truncatable 'false');
...
TRUNCATE tru_ftable;			-- error
--Testcase 865:
ALTER SERVER duckdb_svr OPTIONS (DROP truncatable);
--Testcase 866:
ALTER FOREIGN TABLE tru_ftable OPTIONS (SET truncatable 'false');
TRUNCATE tru_ftable;			-- error
--Testcase 867:
...
-- DuckDB FDW support TRUNCATE command by executing DELETE statement without WHERE clause.
-- In order to delete records in parent and child table subsequently,
-- DuckDB FDW executes "PRAGMA foreign_keys = ON" before executing DELETE statement.
...
-- ===================================================================
-- -- Test case relative with option application_name is not suitable for DuckDB FDW.
-- -- Because this option is in libpq of postgres.
-- -- Change application_name of remote connection to special one
-- -- so that we can easily terminate the connection later.
-- ALTER SERVER duckdb_svr OPTIONS (application_name 'fdw_retry_check');
...
-- =============================================================================
-- test connection invalidation cases and duckdb_fdw_get_connections function
-- =============================================================================
-- Let's ensure to close all the existing cached connections.
--Testcase 880:
SELECT 1 FROM duckdb_fdw_disconnect_all();
-- No cached connections, so no records should be output.
--Testcase 881:
SELECT server_name FROM duckdb_fdw_get_connections() ORDER BY 1;
-- This test case is for closing the connection in duckdbfdw_xact_callback
BEGIN;
...
-- List all the existing cached connections. duckdb_svr and duckdb_svr3 should be
-- output.
--Testcase 884:
SELECT server_name FROM duckdb_fdw_get_connections() ORDER BY 1;
...
-- they are just marked as invalid in duckdbfdw_inval_callback.
--Testcase 885:
ALTER SERVER duckdb_svr OPTIONS (ADD keep_connections 'off');
--Testcase 886:
DROP SERVER duckdb_svr3 CASCADE;
-- List all the existing cached connections. duckdb_svr and duckdb_svr3
-- should be output as invalid connections. Also the server name for
-- duckdb_svr3 should be NULL because the server was dropped.
--Testcase 887:
SELECT * FROM duckdb_fdw_get_connections() ORDER BY 1;
-- The invalid connections get closed in duckdbfdw_xact_callback during commit.
COMMIT;
--Testcase 888:
ALTER SERVER duckdb_svr OPTIONS (DROP keep_connections);
...
-- =======================================================================
-- test duckdb_fdw_disconnect and duckdb_fdw_disconnect_all functions
-- =======================================================================
BEGIN;
...
-- List all the existing cached connections. duckdb_svr and duckdb_svr2 should be
-- output.
--Testcase 892:
SELECT server_name FROM duckdb_fdw_get_connections() ORDER BY 1;
-- Issue a warning and return false as duckdb_svr connection is still in use and
-- can not be closed.
--Testcase 893:
SELECT duckdb_fdw_disconnect('duckdb_svr');
-- List all the existing cached connections. duckdb_svr and duckdb_svr2 should be
-- output.
--Testcase 894:
SELECT server_name FROM duckdb_fdw_get_connections() ORDER BY 1;
...
COMMIT;
-- Ensure that duckdb_svr2 connection is closed.
--Testcase 898:
SELECT 1 FROM duckdb_fdw_disconnect('duckdb_svr2');
--Testcase 899:
SELECT server_name FROM duckdb_fdw_get_connections() WHERE server_name = 'duckdb_svr2';
-- Return false as duckdb_svr2 connection is closed already.
--Testcase 900:
SELECT duckdb_fdw_disconnect('duckdb_svr2');
-- Return an error as there is no foreign server with given name.
--Testcase 901:
SELECT duckdb_fdw_disconnect('unknownserver');
-- Let's ensure to close all the existing cached connections.
--Testcase 902:
SELECT 1 FROM duckdb_fdw_disconnect_all();
-- No cached connections, so no records should be output.
...
--Testcase 906:
CREATE USER MAPPING FOR regress_multi_conn_user1 SERVER duckdb_svr;
--Testcase 907:
CREATE USER MAPPING FOR regress_multi_conn_user2 SERVER duckdb_svr;

BEGIN;
-- Will cache duckdb_svr connection with user mapping for regress_multi_conn_user1
--Testcase 908:
...
-- Will cache duckdb_svr connection with user mapping for regress_multi_conn_user2
--Testcase 911:
...
-- Should output two connections for duckdb_svr server
--Testcase 914:
SELECT server_name FROM duckdb_fdw_get_connections() ORDER BY 1;
COMMIT;
-- Let's ensure to close all the existing cached connections.
...
-- Clean up
--Testcase 917:
DROP USER MAPPING FOR regress_multi_conn_user1 SERVER duckdb_svr;
--Testcase 918:
DROP USER MAPPING FOR regress_multi_conn_user2 SERVER duckdb_svr;
--Testcase 919:
...
--Testcase 921:
ALTER SERVER duckdb_svr OPTIONS (keep_connections 'off');
-- connection to duckdb_svr server is closed at the end of xact
...
SELECT server_name FROM duckdb_fdw_get_connections() ORDER BY 1;
--Testcase 924:
ALTER SERVER duckdb_svr OPTIONS (SET keep_connections 'on');
...
--Testcase 925:
CREATE SERVER batch10 FOREIGN DATA WRAPPER duckdb_fdw OPTIONS( batch_size '10' );
...
--Testcase 935:
CREATE FOREIGN TABLE ftable ( x int OPTIONS (key 'true') ) SERVER duckdb_svr OPTIONS ( table 'batch_table', batch_size '10' );
--Testcase 936:
...
--Testcase 944:
CREATE FOREIGN TABLE ftable ( x int OPTIONS (key 'true') ) SERVER duckdb_svr OPTIONS ( table 'batch_table', batch_size '100000' );
--Testcase 945:
...
--Testcase 949:
CREATE FOREIGN TABLE ftable ( x int ) SERVER duckdb_svr OPTIONS ( table 'batch_table', batch_size '1' );
--Testcase 950:
...
CREATE FOREIGN TABLE inv_bsz (c1 int )
	SERVER duckdb_svr OPTIONS (batch_size '100$%$#$#');
