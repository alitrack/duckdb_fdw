...
# duckdb_fdw 技术演进路线图 (Technical Roadmap)

本文档是对主研究报告的补充，旨在为 `duckdb_fdw` 作者提供**工程落地**层面的具体指导。报告中指出的战略方向需要落实到具体的 C++ 代码实现和 Postgres API 调用上。

## 路径 A：打造极致的 ETL 加速器 (The Turbo Loader)

### 战略目标
放弃与 `pg_duckdb` 在复杂分析查询（Aggregates/Joins）上的性能竞争，转而成为 **Postgres 生态中最快的数据加载工具**。目标是让用户用 SQL 就能以接近 `pg_bulkload` 的速度将 Parquet/Iceberg 数据导入 Postgres。

### 技术现状与瓶颈
- **当前瓶颈**：目前的 FDW 写操作（Write）通常是逐行调用 `ExecForeignInsert`。对于百万级数据导入，函数调用开销巨大。
- **Postgres 能力**：Postgres 14 引入了 `GetForeignModifyBatchSize` 和 `ExecForeignBatchInsert` API，允许 FDW 一次性接收多行元组。

### 工程实施路线图

#### 1. 实现批量插入 API (Postgres 14+)
- **任务**：在 `duckdb_fdw` 中实现 `GetForeignModifyBatchSize` 回调函数。
- **逻辑**：返回一个合理的批次大小（例如 1000 或 10000），告知 Postgres 执行器“我可以一次处理这么多行”。
- **任务**：实现 `ExecForeignBatchInsert` 回调。
    - **输入**：`ResultRelInfo *`, `TupleTableSlot **` (元组数组), `TupleTableSlot **` (PlanSlots).
    - **处理**：
        1.  不要每一行都调用 DuckDB 的 `INSERT` SQL。
        2.  **关键优化**：使用 DuckDB 的 **Appender Interface** (`duckdb_appender_create`, `duckdb_append_row` 等)。Appender 是 DuckDB 提供的最高效数据写入方式，绕过了 SQL 解析器。
        3.  将 Postgres 传递过来的 `TupleTableSlot` 数组，紧凑地通过 Appender 写入 DuckDB。
        4.  执行 `duckdb_appender_flush` 提交批次。

#### 2. 优化数据类型转换路径
- **现状**：`Postgres Datum` -> `C++ Type` -> `DuckDB Value` 转换路径冗长。
- **优化**：
    - 针对常见类型（INT, BIGINT, DOUBLE, DATE），直接进行内存拷贝（Memcpy），避免昂贵的构造函数开销。
    - 在 Appender 层面预分配缓冲区。

#### 3. 预期收益
- 写入性能提升 10x-50x。
- 成为将 Parquet/S3 数据“物化”到 Postgres 本地表的首选工具。

---

## 路径 B：边缘联邦与数据湖直连 (The Federation Specialist)

### 战略目标
成为连接 Postgres 与现代数据湖（Iceberg, Delta Lake）的通用桥梁。利用 DuckDB 对云原生格式的强大支持，让 Postgres 能“看”到 S3 上的海量数据。

### 技术实施路线图

#### 1. 深度集成 DuckDB Secrets Manager
- **现状**：用户目前需要在 FDW `OPTIONS` 中明文写 Access Key，不安全且麻烦。
- **改进**：
    - 引入新的 FDW 选项 `duckdb_secret`。
    - 在连接建立时，自动执行 `CREATE SECRET ...` 语句，利用 DuckDB 原生能力管理凭证。
    - 支持 IAM Role 传递（`CHAIN` provider），简化 AWS 环境下的配置。

#### 2. 实现 Iceberg 元数据自动发现
- **痛点**：用户不想手动 `CREATE FOREIGN TABLE` 定义 100 个列。
- **方案**：利用 Postgres 的 `IMPORT FOREIGN SCHEMA` 机制。
- **实现**：
    1.  实现 `ImportForeignSchema` 回调。
    2.  在 DuckDB 侧执行 `SELECT * FROM iceberg_scan('s3://bucket/table', ALLOW_MOVED_PATHS=true) LIMIT 0` 获取 Schema。
    3.  将 DuckDB 类型自动映射为 Postgres 类型。
    4.  自动生成 `CREATE FOREIGN TABLE` 语句。

#### 3. 谓词下推的极致优化
- **现状**：FDW 只能下推基本的 `WHERE` 子句。
- **优化**：
    - 确保 `File Filters`（行组过滤）生效。DuckDB 需要看到明确的过滤条件才能跳过 Parquet Row Groups。
    - 重点测试 `Timestamp` 和 `Date` 类型的下推逻辑，确保 Postgres 的时区处理不会阻碍 DuckDB 的谓词下推（这是常见的坑）。

---

## 路径 C：探索 Zero-Copy (实验性/高风险)

### 战略目标
尝试打破 FDW 的“元组序列化”诅咒。

### 核心思路
虽然标准 FDW API 要求返回 Tuple，但我们可以利用 **Postgres Custom Scan (自定义扫描)** 接口。

1.  **Arrow C Data Interface**: DuckDB C API (`duckdb_query_arrow_array`) 可以输出 Arrow 格式的内存结构。
2.  **Postgres 侧**：虽然 Postgres 执行器需要 Tuple，但如果配合 `pg_analytics` 或类似的扩展，或者如果只是为了将数据导出给客户端（如通过 Arrow Flight SQL），则有机会绕过 Tuple 构造。
3.  **中间状态方案**：
    - 在 `IterateForeignScan` 内部，一次性从 DuckDB 拉取一个 `ArrowRecordBatch` (比如 2048 行)。
    - 将这个 Batch 缓存在 FDW 的私有内存中。
    - 当 Postgres 请求下一行时，直接从缓存的 Arrow 数组中读取值填入 Slot，而不是每次都去调用 DuckDB API。
    - **收益**：减少了 2048 次 DuckDB C API 调用开销（Context Switch），仅保留内存拷贝开销。这将带来显著的读性能提升。
