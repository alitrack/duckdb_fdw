...
CREATE SERVER srv_parquet_train FOREIGN DATA WRAPPER duckdb_fdw OPTIONS (database '/tmp/duckdb_fdw_dataset_parquet.db', extensions 'httpfs,parquet');
CREATE FOREIGN TABLE view_parquet_train (
    station_name TEXT,
    train_number TEXT,
    arrival_time TEXT
) SERVER srv_parquet_train OPTIONS (table 'read_parquet(''https://blobs.duckdb.org/train_services.parquet'')');
...
CREATE SERVER srv_iceberg FOREIGN DATA WRAPPER duckdb_fdw OPTIONS (
    database '/tmp/duckdb_fdw_dataset_iceberg.db',
...
CREATE FOREIGN TABLE view_iceberg (
    p_partkey BIGINT,
    p_name TEXT
) SERVER srv_iceberg OPTIONS (table '__my_resource__.tpch10.part');
